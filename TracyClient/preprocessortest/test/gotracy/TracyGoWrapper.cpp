...
void SetZoneContext( int c, TracyZoneData* data ) { TracyCZoneCtxMap[c] = data; }

void DelZoneContext( int c )
{
    auto it = TracyCZoneCtxMap.find( c );
    if( it != TracyCZoneCtxMap.end() ) TracyCZoneCtxMap.erase( it );
}
...
int IsZoneContextExist( int c )
{
    ...
    if( search == TracyCZoneCtxMap.end() ) return 0;
    return 1;
}

void GoTracySetThreadName( const char* name ) { ___tracy_set_thread_name( name ); }
...
void GoTracyMessageL( char* msg ) { ___tracy_emit_messageL( msg, 1 ); }

void GoTracyMessageLC( char* msg, uint32_t color ) { ___tracy_emit_messageLC( msg, color, 1 ); }

void GoTracyFrameMark() { ___tracy_emit_frame_mark( (char*)0 ); }

void GoTracyFrameMarkName( char* name ) { ___tracy_emit_frame_mark( name ); }

void GoTracyFrameMarkStart( char* name ) { ___tracy_emit_frame_mark_start( name ); }

void GoTracyFrameMarkEnd( char* name ) { ___tracy_emit_frame_mark_end( name ); }

void GoTracyPlotFloat( char* name, float val ) { ___tracy_emit_plot_float( name, val ); }

void GoTracyPlotDoublet( char* name, double val ) { ___tracy_emit_plot( name, val ); }

void GoTracyPlotInt( char* name, int val ) { ___tracy_emit_plot_int( name, val ); }

void GoTracyMessageAppinfo( char* info ) { ___tracy_emit_message_appinfo( info, strlen( info ) ); }
