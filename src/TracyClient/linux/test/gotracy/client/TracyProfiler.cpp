...
#ifdef __MINGW32__
#include <intrin.h>
static inline int __get_cpuid( unsigned int leaf, unsigned int* eax, unsigned int* ebx, unsigned int* ecx,
                               unsigned int* edx )
{
    int info[4];
    __cpuid( info, leaf );
...
constexpr LOGICAL_PROCESSOR_RELATIONSHIP RelationProcessorDie = static_cast<LOGICAL_PROCESSOR_RELATIONSHIP>( 3 );
#endif
#endif
